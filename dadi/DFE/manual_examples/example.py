...
import pickle
spectra = pickle.load(open("example_spectra.sp"))
...
lower_bound = [1e-3, 1e-2, 0]
upper_bound = [1, 50000., 1]
p0 = dadi.Misc.perturb_params(sel_params, lower_bound=lower_bound,
                              upper_bound=upper_bound)
popt = Selection.optimize_log(p0, data, misid_func, Selection.gamma_dist, 
                              theta_ns, lower_bound=lower_bound, 
                              upper_bound=upper_bound, verbose=len(sel_params), 
                              maxiter=30)
#
...
data = dadi.Spectrum.from_file('example.sfs')
ppos = 0.1
func_ex = dadi.Numerics.make_extrap_func(two_epoch_sel)
sel_data = theta_ns*func_ex(tuple(demog_params) + (5,), ns, pts_l)

data_pos = (1-ppos)*data + ppos*sel_data

sel_params = [0.2, 1000., 0.2, 2]
lower_bound = [1e-3, 1e-2, 0, 0]
upper_bound = [1, 50000., 1, 50]
p0 = dadi.Misc.perturb_params(sel_params, lower_bound=lower_bound,
                              upper_bound=upper_bound)
popt = Selection.optimize_log(p0, data_pos, spectra.integrate_point_pos, 
                              Selection.gamma_dist, 
                              theta_ns, lower_bound=lower_bound, 
                              fixed_params=[None,None,None,None],
                              upper_bound=upper_bound, verbose=len(sel_params), 
                              func_args = [func_ex], maxiter=30)
