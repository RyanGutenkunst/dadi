...
__global__ void calc_s(double *phi, double *yy, int L, int M, double *s){
    int ii = (blockIdx.x*blockDim.x + threadIdx.x) / (M-1);
    int jj = (blockIdx.x*blockDim.x + threadIdx.x) % (M-1);

    if(ii < L){
        s[ii*(M-1) + jj] = (phi[ii*M+jj+1] - phi[ii*M+jj])/(yy[jj+1]-yy[jj]);
    }
}

__global__ void calc_c1(double *phi, double *yy, double *s_yy, int ny, int L, int M, double *s){
    //c1_yy = (phi[:,:-1] - s_yy*yy[nuax,:-1])/(ny+1)
    int ii = (blockIdx.x*blockDim.x + threadIdx.x) / (M-1);
    int jj = (blockIdx.x*blockDim.x + threadIdx.x) % (M-1);

    if(ii < L){
        s[ii*(M-1) + jj] = (phi[ii*M+jj] - s_yy[ii*(M-1)+jj]*yy[jj])/(ny+1.);
    }
}

__global__ void combine_terms(double *term1, double *term2, int n, int L, int M, double *out){
    //c1_yy = (phi[:,:-1] - s_yy*yy[nuax,:-1])/(ny+1)
    int ii = (blockIdx.x*blockDim.x + threadIdx.x) / M;
    int jj = (blockIdx.x*blockDim.x + threadIdx.x) % M;

    if(ii < L){
        out[ii*M + jj] = term1[ii*M+jj] + term2[ii*M+jj]*(ii+1.)/((n+1)*(n+2));
    }
}