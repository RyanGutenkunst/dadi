...
def two_epoch_sel(params, ns, pts):
    ...
    Parameters:
        params (tuple): (T_div, T_WGD, nu, H, gamma)
    ...
    return fs
two_epoch_sel.__param_names__ = ['T_div', 'T_WGD', 'nu', 'H', 'gamma']

def two_epoch_sel_noHE(params, ns, pts):
    """
    Two epoch model of allotetraploid formation where the 
    diploid progenitors diverge for 1 diffusion unit and then the
    allotetraploid population splits and maintains a size of nu.
    ...
    Returns:
        fs (Spectrum): The resulting (collapsed) frequency spectrum.
    """
    T_div, T_WGD, nu, gamma = params
    fs = two_epoch_sel((T_div, T_WGD, nu, 0, gamma), ns, pts)
    return fs
two_epoch_sel_noHE.__param_names__ = ['T_div', 'T_WGD', 'nu', 'gamma']
...
def bottlegrowth_sel(params, ns, pts):
    ...
    """
    T_div, T_WGD, nuWGD, nuF, H, gamma = params
    ...
    fs = Spectrum.from_phi(phi, ns, (xx,xx))
    return fs
bottlegrowth_sel.__param_names__ = ["T_div", "T_WGD", "nuWGD", "nuF", "H", "gamma"]

def bottlegrowth_sel_noHE(params, ns, pts):
    """
    Bottlegrowth model of allotetraploid formation where the 
    allotetraploid population starts with size nuWGD and 
    grows exponentially to a size of nuF
    ...
    Returns:
        fs (Spectrum): The resulting frequency spectrum.
    """
    T_div, T_WGD, nuWGD, nuF, gamma = params
    fs = bottlegrowth_sel((T_div, T_WGD, nuWGD, nuF, 0, gamma), ns, pts)
    return fs
bottlegrowth_sel_noHE.__param_names__ = ["T_div", "T_WGD", "nuWGD", "nuF", "gamma"]
...
def three_epoch_sel(params, ns, pts):
    ...
    Returns:
        fs (Spectrum): The resulting frequency spectrum.
    """
    T_div, T_WGD, TF, nuWGD, nuF, H, gamma = params
    alloaflag = PolyInt.PloidyType.ALLOa
    allobflag = PolyInt.PloidyType.ALLOb
    xx = Numerics.default_grid(pts)
    phi = PhiManip.phi_1D(xx, gamma=gamma)
    ...
    fs = Spectrum.from_phi(phi, ns, (xx,xx))
    return fs
three_epoch_sel.__param_names__ = ['T_div', 'T_WGD', 'TF', 'nuWGD', 'nuF', 'H', 'gamma']


def three_epoch_sel_noHE(params, ns, pts):
    """
    Three epoch model of allotetraploid formation where the 
    allotetraploid population splits, maintains a size of nuWGD for T_WGD, 
    and then changes size again to nuF for a period of TF.
    ...
    Returns:
        fs (Spectrum): The resulting frequency spectrum.
    """
    T_div, T_WGD, TF, nuWGD, nuF, gamma = params
    fs = three_epoch_sel((T_div, T_WGD, TF, nuWGD, nuF, 0, gamma), ns, pts)
    return fs
three_epoch_sel_noHE.__param_names__ = ['T_div', 'T_WGD', 'TF', 'nuWGD', 'nuF', 'gamma']
...
def bottleneck_asym_mig_w_dips_sel(params, ns, pts):
    ...
    Parameters:
        params (tuple): (T_div, T_WGD, nu_allo, H, m31, m42)
            ...
            - gamma: population-scaled selection coefficient (= 2*Na*s)
                Note that this is used for both the diploid and allotetraploid populations.
    ...
    """
    T_div, T_WGD, nu_allo, H, m31, m42, gamma = params
    ...
    phi = PolyInt.two_pops(phi, xx, T_div-T_WGD, sel_dict1={"gamma": gamma}, sel_dict2={"gamma": gamma})
    ...
    phi = PolyInt.four_pops(phi, xx, T_WGD, m31=m31, m42=m42, m34=H, m43=H,
                            sel_dict1={"gamma": gamma}, sel_dict2={"gamma": gamma},
                            sel_dict3={"gamma": gamma}, sel_dict4={"gamma": gamma},
                            nu1=1, nu2=1, nu3=nu_allo, nu4=nu_allo,
                            ploidyflag3=alloaflag, ploidyflag4=allobflag)
    
    fs = Spectrum.from_phi(phi, ns, (xx,xx,xx,xx))
    return fs
bottleneck_asym_mig_w_dips_sel.__param_names__ = ['T_div', 'T_WGD', 'nu_allo', 'H', 'm31', 'm42', 'gamma']
...
def bottleneck_mig_w_dips_sel(params, ns, pts):
    ...
    Raises:
        ValueError: If `params` does not contain the expected number of elements.
    """
    T_div, T_WGD, nu_allo, H, m, gamma = params
    return bottleneck_asym_mig_w_dips_sel((T_div, T_WGD, nu_allo, H, m, m, gamma), ns, pts)
bottleneck_mig_w_dips_sel.__param_names__ = ['T_div', 'T_WGD', 'nu_allo', 'H', 'm', 'gamma']
...
def bottleneck_w_dips_sel(params, ns, pts):
    ...
    Raises:
        ValueError: If `params` does not contain the expected number of elements.
    """
    T_div, T_WGD, nu_allo, H, gamma = params
    return bottleneck_asym_mig_w_dips_sel((T_div, T_WGD, nu_allo, H, 0, 0, gamma), ns, pts)
bottleneck_w_dips_sel.__param_names__ = ['T_div', 'T_WGD', 'nu_allo', 'H', 'gamma']

def bottleneck_noHE_w_dips_sel(params, ns, pts):
    """
    Three population (4D) model of allotetraploid formation where 
    the diploid progenitors diverge and then a WGD event occurs 
    ...
    Raises:
        ValueError: If `params` does not contain the expected number of elements.
    """
    T_div, T_WGD, nu_allo, gamma = params
    return bottleneck_asym_mig_w_dips_sel((T_div, T_WGD, nu_allo, 0, 0, 0, gamma), ns, pts)
bottleneck_noHE_w_dips_sel.__param_names__ = ['T_div', 'T_WGD', 'nu_allo', 'gamma']
