import dadi
import dadi.TwoLocusnos 
import numpy as np

"""
Examples for
1. Loading and manipulating two-locus spectrum
2. Calculating expected spectrum using demography functions
...
## 2. Create a two locus spectrum under demographic
...
# note that the first time computing the equilibrium spectrum for a give dt, pts
# can take a while, which is why we cache those spectra
spectra = {}
for dt in dts:
...
# extrap_dt_pts takes a dictionary in the form spectra[dts][pts] and extrapolates the 
# calculated spectra over dt and pts to output the expected frequency spectrum
...
fs = dadi.TwoLocus.TLSpectrum.from_file('example_observed_fs.fs')
# the inference/optimize functions takes a list of two-locus frequency spectra, and a list of 
# the rho values for each of the spectra in the list, in the same order as the spectra in the list
# here, we only are optimizing over a single frequency spectrum for rho = 1
data_list = [fs]
rho_list = [1.0]
# we know that the sample spectrum was generate under a demography with nu = 2.0, T = 0.1, so for 
# illustration purposes we pick initial values close to the true values
initial_guess = dadi.Misc.perturb_params([2.0,0.1], fold=0.2)

gridpts = [40,50,60]
dts = [0.005,0.0025,0.001]

# do_calculation computes the expected frequency spectrum for a given sample size and rho
...
opt_params = dadi.TwoLocus.inference.optimize_log_fmin(initial_guess, data_list, model_func, 
                        gridpts, dts, rhos=rho_list, verbose=1)
