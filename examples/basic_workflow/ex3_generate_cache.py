...
fs = dadi.Spectrum.from_file('data/fs/1KG.YRI.CEU.20.nonsynonymous.snps.unfold.fs')
ns = fs.sample_sizes
...
demo_sel_model = DFE.DemogSelModels.split_mig
...
# on the HPC.
cache2d = DFE.Cache2D(popt, ns, demo_sel_model, pts=pts_l, gamma_bounds=[1e-2, 200], gamma_pts=10, mp=True)
...
fid = open('results/example_2d_cache.bpkl', 'wb')
