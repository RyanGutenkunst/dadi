...
def test_1d_freeze():
    ...
    phi_frozen = one_pop(phi_orig, xx, T=0.1, nu=0.1, gamma=0.2, 
                         frozen=True)
    assert(numpy.allclose(phi_frozen, phi_orig, rtol=1e-6))

def test_2d_freeze_marginal():
    """
    ...
    assert(numpy.allclose(phi_orig_just1[1:-1], 
                                phi_frozen1_just1[1:-1], 
                                rtol=1e-4))
    assert(numpy.allclose(phi_orig_just2[1:-1], 
    ...
    assert(numpy.allclose(phi_frozen2_just1[1:-1], 
                                phi_decouple_just1[1:-1], 
                                rtol=5e-2))
    assert(numpy.allclose(phi_frozen1_just2[1:-1], 
                                phi_decouple_just2[1:-1], 
                                rtol=5e-2))

def test_3d_freeze_marginal():
    """
    ...
    assert(numpy.ma.allclose(fs_orig.marginalize([1,2]),
                          fs_frozen1.marginalize([1,2]), 
                          rtol=1e-2))
    assert(numpy.ma.allclose(fs_orig.marginalize([0,2]),
                          fs_frozen2.marginalize([0,2]), 
                          rtol=1e-2))
    assert(numpy.ma.allclose(fs_orig.marginalize([0,1]),
                          fs_frozen3.marginalize([0,1]), 
                          rtol=1e-2))

    assert(numpy.ma.allclose(fs_frozen1.marginalize([0]),
                          fs_decouple1.marginalize([0]), 
                          rtol=1e-1))
    assert(numpy.ma.allclose(fs_frozen2.marginalize([1]),
                          fs_decouple2.marginalize([1]), 
                          rtol=1e-1))
    assert(numpy.ma.allclose(fs_frozen3.marginalize([2]),
...
def test_2d_correctness():
    ...
    fs_stored = dadi.Spectrum.from_file('tests/test_2D_frozen1.fs')
    assert(numpy.ma.allclose(fs, fs_stored, rtol=1e-2))
