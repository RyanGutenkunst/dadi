...
My preferred workflow involves one window editing a Python script (e.g. \py{script.py}) and another running an IPython session.
...
\subsection{Fixed $\boldsymbol{\theta}$}\label{sec:fixed_theta}
...
In our experience, good results are obtained by using grid sizes that are a multiple of 10, with the smallest grid size slightly larger than the largest population sample size.
...
%s\section{Frequently asked questions}
